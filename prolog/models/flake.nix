{
  outputs = inputs:
    ...
    flakes.makeFlake {
      inputs = { inherit (flakes.all) nixpkgs devshell codium drv-tools; };
      perSystem = { inputs, system }:
        let
          pkgs = inputs.nixpkgs.legacyPackages.${system};
          inherit (inputs.codium.lib.${system}) mkCodium writeSettingsJSON;
          inherit (inputs.codium.lib.${system}) extensions extensionsCommon settingsCommonNix;
          ...
          packages = {
            ...
            writeSettings = writeSettingsJSON {
              prolog."prolog.executablePath" = "${getExe pkgs.swiProlog}";
            };
          } // (mkShellApps {
            run = {
