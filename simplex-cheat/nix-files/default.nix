{ nixpkgs, system, my-codium, drv-tools, devshell, haskell-tools, flakes-tools }:
let
  ...
  inherit (haskellTools ghcVersion override (ps: [ ps.myPackage ]) myPackageDepsBin)
  ...
  writers = (import ./writers.nix {
    inherit drv-tools pkgs system;
  });
...
in
{
  inherit defaultShell flakesTools writers;
}
